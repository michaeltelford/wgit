...
class TestUrl < TestHelper
  ...
  def test_prefix_scheme
    ...
    assert_equal "https://my_server",
                 Wgit::Url.new("my_server").prefix_scheme(:https)
    assert_equal "http://my_server",
                 Wgit::Url.new("//my_server").prefix_scheme(:http)
    assert_equal "https://my_server",
                 Wgit::Url.new("//my_server").prefix_scheme(:https)
    assert_equal "http://my_server", Wgit::Url.new("http://my_server").prefix_scheme
    ex = assert_raises(StandardError) do
      Wgit::Url.new("my_server").prefix_scheme(:ftp)
    end
  ...
  def test_relative?
    ...
    assert_equal "Unknown opts param: :foo, use one of: [:origin, :host, :domain, :brand]",
                 e.message

    e = assert_raises(StandardError) do
      Wgit::Url.new("http://example.com").relative?(foo: "/")
    end
    assert_equal "Invalid opts param value, it must be absolute, containing a protocol scheme and domain (e.g. http://example.com): /",
                 e.message

    e = assert_raises(StandardError) do
      Wgit::Url.new("http://example.com").relative?(brand: "http://example")
    end
    assert_equal "Invalid opts param value, it must be absolute, containing a protocol scheme and domain (e.g. http://example.com): http://example",
                 e.message
  end
  ...
  def test_crawled=
    ...
    url = Wgit::Url.new "http://www.google.co.uk", crawled: true,
date_crawled: Time.now
  ...
  def test_redirects=
    ...
    assert(url.redirects.all? do |k, v|
             k.instance_of?(Wgit::Url) && v.instance_of?(Wgit::Url)
           end)
  ...
  def test_to_query_hash
    ...
    assert_equal({ "q" => "ruby", "page" => "2", "limit" => "ten" },
                 url.to_query_hash)
    assert_equal({ q: "ruby", page: "2", limit: "ten" },
                 url.to_query_hash(symbolize_keys: true))
