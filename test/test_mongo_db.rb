...
class TestMongoDB < TestHelper
  ...
  def test_initialize
    ...
    reset_connection_string do
      ...
      assert_equal "connection_string and ENV['WGIT_CONNECTION_STRING'] are nil",
                   e.message
    end
  end
  ...
  def test_insert__urls
    ...
    assert_equal "obj must be a Wgit::Url or Wgit::Document, not: TrueClass",
                 e.message
  ...
  def test_get__empty
    ...
    assert_equal "obj must be a Wgit::Url or Wgit::Document, not: Integer",
                 ex.message
